...
(function (global) {
  ...
  function showCard(e, i) {
    ...
    var card = el('div', { class: 'show' + (e._id === activeId ? ' active' : '') }, [
      el('div', { class: 'show-h' }, [
        ...
        el('button', {
          class: 'kill', type: 'button', title: 'Verwijder deze beurs', 'aria-label': 'Verwijder deze beurs', text: '✕',
          onclick: function () {
      ...
      field('Plaats', city, openCity ? 'Staat nog open — drukt blauw af' : ''),
      el('div', { class: 'pair' }, [
        field('Datum', start),
  ...
  var GROUPS = [
    ['Vlak', [
      ['bg', 'Achtergrond'], ['pattern', 'Lijnen op de achtergrond'],
      ['sweep', 'Rode zwaai'], ['sweepDark', 'Zwarte hoek']
    ]],
    ['Kop bovenaan', [
      ['logoRed', 'Logo: rood (alleen placeholder)'], ['logoInk', 'Logo: zwart (alleen placeholder)'],
      ['knot', 'Ornament onder logo'],
      ['heading', 'Kop (BEURZEN)'], ['eyebrow', 'Rode regel eronder'], ['span', 'Periode'],
      ['headRule', 'Lijn onder de kop']
    ]],
    ['De lijst', [
      ['date', 'Datum'], ['name', 'Naam beurs'], ['city', 'Plaats'], ['cityMark', 'Balletje voor de plaats'],
      ['day', 'Weekdag'],
      ['rowRule', 'Lijn tussen beurzen'], ['bandYear', 'Jaar / maand'],
      ['bandLabel', 'Seizoen / jaartal'], ['bandRule', 'Lijn van de balk'],
      ['bandTick', 'Streepje bij de balk'], ['marker', 'Nog invullen (‹CITY›)']
    ]],
    ['Onderaan', [
  ...
  function swatch(key, label) {
    ...
    var back = el('button', {
      class: 'undo', type: 'button', title: 'Terug naar de huisstijlkleur', 'aria-label': label + ' terugzetten', text: '↺',
      onclick: function () {
  ...
  function kleurenSection() {
    ...
    GROUPS.forEach(function (g, gi) {
      ...
      if (gi === 0) {
        box.appendChild(el('button', {
          class: 'btn wide ghost', type: 'button', text: 'Andere lijnen',
          title: 'De lijnen op de achtergrond opnieuw verdelen',
          onclick: function () {
            var st = S.load();
            S.setPath('meta.bgSeed', ((st.meta.bgSeed || 0) + 1) % 1000, true);
            renderer().render();
          }
        }));
      }
    ...
    box.appendChild(el('p', { class: 'hint', style: 'margin:10px 0 0', text:
      'Zodra het echte logobestand erin zit, doen de twee logo-kleuren niets meer ' +
      'met het logo: dat is het aangeleverde bestand en dat wordt niet hertint.' }));
    return box;
  }
  ...
  function warnings(layout) {
    ...
    if (open.length) {
      out.push(['Nog invullen',
        open.length + ' plaats(en) staan nog als ‹CITY› en drukken blauw af: ' +
    ...
    /* The lockup is red and black. Until the supplied file is in
       place the placeholder follows the two logo swatches; once it
       is, the file is what it is and a dark field needs a light
       version of the logo. */
    var LOGO_INK = global.Brand.LOGO_FILE ? '#111111' : th.logoInk;
    if (ratio(LOGO_INK, th.bg) < 3) {
      out.push(['Logo valt weg',
        'Het zwart van het logo staat op ' + ratio(LOGO_INK, th.bg).toFixed(1) + ':1 tegen deze ' +
        'achtergrond. Voor een donkere kaart is een lichte versie van het logo nodig.', false]);
    }
  ...
  function paint(layout) {
    ...
    TABS.forEach(function (t) {
      nav.appendChild(el('button', {
        class: 'pnav-b' + (section === t[0] ? ' on' : ''), type: 'button', text: t[1],
        'aria-pressed': section === t[0] ? 'true' : 'false',
