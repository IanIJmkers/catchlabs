...
(function (global) {
  ...
  function build() {
    ...
    var hits = [], html = K.ground(F, st) + '<div class="plane-3">' + head.html;
    var bottom = head.bodyTop;

    /* The year is only printed on the month that turns it: SEPTEMBER
       and OKTOBER need no year, DECEMBER and JANUARI 2027 do. Both
       month labels set at the same size, the smaller of the two. */
    var subOf = function (b) { return b.month.y !== months[0].y ? String(b.month.y) : ''; };
    var fyAll = Math.min.apply(null, blocks.map(function (b) {
      return K.bandSize(pitch, L.colW, S.MONTHS[b.month.m], subOf(b));
    }));
    ...
    L.cols.forEach(function (col, ci) {
      ...
      col.forEach(function (b) {
        html += K.band(x, y, L.colW,
          { label: S.MONTHS[b.month.m], sub: subOf(b), fy: fyAll }, pitch);
