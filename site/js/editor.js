...
(function (global) {
  ...
  /* ---------- scale the artboard to the window ----------
     Three ways to look at it. `fit` shows the whole card, which on a
     laptop is a third of its size and the small type goes with it;
     `width` fills the stage sideways and scrolls, which is where the
     list is actually checked; `1` is the pixel grid the PNG will be. */
  var zoom = 'fit';
  function setZoom(mode) {
    zoom = mode;
    Array.prototype.forEach.call(document.querySelectorAll('[data-zoom]'), function (n) {
      n.classList.toggle('on', n.getAttribute('data-zoom') === mode);
    });
    fitStage();
  }
  function fitStage() {
    ...
    var pad = 56;
    var tools = stage.querySelector('.stage-tools');
    var toolsH = tools ? tools.offsetHeight + 14 : 0;   /* the zoom pill sits under the card */
    var byW = (stage.clientWidth - pad) / PAGE_W;
    var s = zoom === '1' ? 1
          : zoom === 'width' ? Math.min(byW, 1)
          : Math.min(byW, (stage.clientHeight - pad - toolsH) / PAGE_H, 1);
    s = Math.max(s, 0.2);
  ...
  function wireBar(opts) {
    ...
    if ((b = q('#save'))) b.onclick = function () { download('catchlabs-agenda.json', Store.toJSON()); };
    ...
    markFormat();
    Array.prototype.forEach.call(document.querySelectorAll('[data-zoom]'), function (n) {
      n.onclick = function () { setZoom(n.getAttribute('data-zoom')); };
    });
    setZoom(zoom);
  ...
  global.Editor = {
    ...
    fitStage: fitStage, setZoom: setZoom, bindEditables: bindEditables, exportPNG: exportPNG,
